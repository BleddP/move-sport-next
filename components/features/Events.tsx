...
export const Events = ({ events, intro }) => {
  ...
  const perMonth = unique.map((date: string) => {
    const events = parsedDates.filter(
      (event: any) =>
        `${event.parsedDate.month}-${event.parsedDate.year}` === date,
    )

    const thisMonth = parseInt(moment().format('MM'))
    const thisyear = parseInt(moment().format('YYYY'))
    const showEvents = events.find(
      (event: any) =>
        parseInt(moment(event.attributes.date).format('MM')) + 1 >= thisMonth &&
        parseInt(moment(event.attributes.date).format('YYYY')) >= thisyear,
    )
